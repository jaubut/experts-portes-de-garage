...
export default function GarageHouse() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      el.style.setProperty("--door-open", String(pct));
      ticking = false;
    };
  ...
  return (
    <div ref={containerRef} className="w-full max-w-md mx-auto select-none" style={{ "--door-open": "0" } as React.CSSProperties}>
      <svg viewBox="0 0 400 380" xmlns="http://www.w3.org/2000/svg" className="w-full h-auto drop-shadow-2xl">
        ...
        {/* Ground */}
        <rect x="0" y="345" width="400" height="35" fill="#6b8f5e" />
        <rect x="0" y="355" width="400" height="25" fill="#5a7d4e" />
        ...
        {/* Garage door (animated via CSS variable) */}
        <g clipPath="url(#doorClip)">
          <g className="garage-door">
            <rect x="100" y="175" width="200" height="170" fill="#c4c4c4" />
            {[0, 1, 2, 3].map((i) => (
              <g key={i}>
                <rect x="104" y={179 + i * 42} width="192" height="38" fill="#d4d4d4" stroke="#b0b0b0" strokeWidth="1" rx="1" />
                <rect x="110" y={185 + i * 42} width="84" height="26" fill="none" stroke="#bbb" strokeWidth="0.5" rx="1" />
                <rect x="206" y={185 + i * 42} width="84" height="26" fill="none" stroke="#bbb" strokeWidth="0.5" rx="1" />
              </g>
            ))}
      ...
      <style jsx>{`
        .garage-door {
          ...
          transform: scaleY(calc(1 - var(--door-open)));
          will-change: transform;
        }
        .garage-inside {
          opacity: var(--door-open);
          will-change: opacity;
        }
      `}</style>
    </div>
